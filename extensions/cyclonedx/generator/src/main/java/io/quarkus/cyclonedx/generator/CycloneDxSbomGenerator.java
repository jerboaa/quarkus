...
public class CycloneDxSbomGenerator {
    ...
    private boolean generateNativeToolInfo;
    ...
    public CycloneDxSbomGenerator setNativeToolInfo(boolean generateNativeToolInfo) {
        ensureNotGenerated();
        this.generateNativeToolInfo = generateNativeToolInfo;
        return this;
    }
    ...
    private void addToolInfo(Bom bom) {
        ...
        if (getSchemaVersion().getVersion() >= 1.5) {
            ...
            List<Component> toolsComponents = new ArrayList<>();
            toolsComponents.add(toolComponent);
            // Add Quarkus native tool info if necessary.
            // Note that the generated SBOM info only will show up
            // in an native image generated by quarkus iff the generator
            // is Mandrel. Oracle GraalVM will generate the SBOM as part of
            // the native image build.
            if (generateNativeToolInfo) {
                /**
                 * "tools": {
                 * "components": [
                 * {
                 * "type": "library",
                 * "bom-ref": "Oracle:org.graalvm.sdk:nativeimage:25.0.2",
                 * "supplier": {
                 * "name": "Oracle"
                 * },
                 ...
                 * "name": "nativeimage",
                 * "version": "25.0.2",
                 * "purl": "pkg:maven/org.graalvm.sdk/nativeimage@25.0.2"
                 * }
                 * ]
                 * },
                 */
                final Component nativeImageComponent = new Component();
                nativeImageComponent.setType(Component.Type.LIBRARY);
                nativeImageComponent.setName("nativeimage");
                nativeImageComponent.setGroup("org.graalvm.sdk");
                OrganizationalEntity redhat = new OrganizationalEntity();
                redhat.setName("Red Hat, Inc.");
                redhat.setUrls(List.of("https://www.redhat.com/"));
                nativeImageComponent.setSupplier(redhat);
                nativeImageComponent.setVersion("__NI_VERSION__");
                nativeImageComponent.setBomRef("redhat:org.graalvm.sdk:nativeimage:__NI_VERSION__");
                try {
                    PackageURL url = new PackageURL("maven", "org.graalvm.sdk", "nativeimage", "__NI_VERSION__",
                            null, null);
                    nativeImageComponent.setPurl(url);
                } catch (MalformedPackageURLException e) {
                    throw new IllegalStateException(e);
                }
                toolsComponents.add(nativeImageComponent);
            }
            toolInfo.setComponents(toolsComponents);
